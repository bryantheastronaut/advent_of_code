...
# are balanced several more sub-towers. At the bottom of these sub-towers, standing on the bottom disc,
# are other programs, each holding their own disc, and so on. At the very tops of these sub-sub-sub-...-towers,
...
# those programs are holding up any other programs, and are all the tops of their own towers.
...
class TowerStack
  def initialize(dataset)
    @dataset = parse_input(dataset)
    @holding = find_elements_holding()
    @not_bottom = []
  end

  def parse_input(dataset)
    data = dataset.split(/\n/)
    data.map { |d| d.gsub(' -> ', ' ').gsub(',', '').split(' ') }
  end

  def find_elements_holding
    @dataset.select { |d| d.length > 2 }
  end

  def check_for_bottom
    being_held = []
    @holding.each do |a|
      copy = a.clone
      copy.shift(2)
      being_held.push(*copy)
    end
    being_held.uniq
  end

  def run
    held = check_for_bottom()
    options = @dataset.map { |x| x[0] }
    options.reject { |x| held.include?(x) }
  end
