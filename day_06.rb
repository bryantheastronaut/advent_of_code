...
class RedistributionAccessoryDirector # RAD!
  def initialize(data)
    @current_memory_bank = data.split(' ').map(&:to_i)
    @in_memory = []
    @match_idx = 0
  end

  def find_highest_idx
    idx = 0
    highest_value = 0
    @current_memory_bank.each_with_index do |val, index|
  ...
  def process_current_bank
    memory_clone = @current_memory_bank.clone
    @in_memory.push(memory_clone)
    idx = find_highest_idx()
    last_idx = @current_memory_bank.length - 1
    value = @current_memory_bank[idx]
    @current_memory_bank[idx] = 0
    while value > 0
      idx += 1
      idx = 0 if idx > last_idx
      @current_memory_bank[idx] += 1
      value -= 1
    end
  end

  def check_idx_for_dupe
    curr_idx = 0
    arr = @in_memory.shift
    last_idx = @in_memory.length - 1
    @in_memory.each_with_index do |a, i|
      if (arr === a)
        @match_idx = i
      end
    end
  ...
  def run
    while @in_memory.length === @in_memory.uniq.length
      process_current_bank()
    end
    return @in_memory.length - 1
  end

  def run_with_cycle_count
    while @in_memory.length === @in_memory.uniq.length
      process_current_bank()
    end
    while (@match_idx === 0 && @in_memory.length > 0)
      check_idx_for_dupe()
    end
    return @match_idx + 1
  end
...
# Inelegant as fuuuuuucccckkkk. So brute force. 0.o
data = RedistributionAccessoryDirector.new(TEST).run
cycles = RedistributionAccessoryDirector.new(INPUT).run_with_cycle_count
p cycles